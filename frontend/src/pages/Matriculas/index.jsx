...
import { FaDollarSign, FaMagic, FaTrash, FaCheck, FaUndo, FaHandHoldingUsd, FaListOl, FaSearch, FaTimes } from "react-icons/fa";
...
export default function Financeiro() {
  ...
  const carregar = useCallback(async () => {
    try {
      ...
      // Se veio ID pela URL, localiza o aluno e preenche a busca
      if (alunoIdViaUrl && dados.length > 0) {
        ...
        if (encontrado) {
          setBuscaNome(encontrado.aluno?.nome || encontrado.matricula?.aluno?.nome || "");
          setMesFiltro(0); // Mostra histórico completo
        }
      }
    } catch (e) {
  ...
  useEffect(() => {
    const lista = registros.filter((item) => {
      ...
      // 2. Filtro de Datas (Se busca por nome está ativa, ignora mês/ano)
      const matchMes = Number(mesFiltro) === 0 || buscaNome !== "" ? true : dataVenc.getUTCMonth() + 1 === Number(mesFiltro);
      const matchAno = Number(anoFiltro) === 0 || buscaNome !== "" ? true : dataVenc.getUTCFullYear() === Number(anoFiltro);

      // 3. Status e Professor
          ...
          <div className="painel-filtros-linha">
            <div className="input-group" style={{ flex: 1 }}>
              ...
              <div style={{ position: "relative", display: "flex", alignItems: "center" }}>
                ...
                {buscaNome && (
                  <FaTimes
                    style={{ position: "absolute", right: "10px", cursor: "pointer", color: "#999" }}
                    onClick={() => {
                      setBuscaNome("");
                      setSearchParams({});
                    }}
                ...
                filtrados.map((p) => {
                  ...
                  return (
                    <tr key={p.id}>
                      ...
                      <td>
                        <strong>{p.aluno?.nome || p.matricula?.aluno?.nome || "Aluno"}</strong>
